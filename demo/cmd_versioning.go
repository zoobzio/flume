...
switch getTestGroup(customerID) {
	...
	versions := []struct {
		name     string
		contract *plugz.Contract[VersionedPayment, VersionedPaymentKey]
	}{
		{"A (MVP)", vA},
		{"B (Standard)", vB},
		{"C (Enterprise)", vC},
...
hybridContract := plugz.GetContract[VersionedPayment](hybridKey)
...
blackFridayContract := plugz.GetContract[VersionedPayment](blackFridayKey)
