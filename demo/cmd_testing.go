...
prodContract := plugz.GetContract[EmailMessage](prodKey)
...
testContract := plugz.GetContract[EmailMessage](testKey)
...
hybridContract := plugz.GetContract[EmailMessage](hybridKey)
...
errorContract := plugz.GetContract[EmailMessage](errorKey)
...
perfContract := plugz.GetContract[EmailMessage](perfKey)
...
snapshotContract := plugz.GetContract[EmailMessage](snapshotKey)
snapshotContract.Register(
	...
	pp.SubSection("🎯 Testing Benefits with plugz")
	...
	pp.Info("plugz: Create a test universe")
	pp.Info("")
	pp.Info("Traditional: Complex mocking frameworks")
	pp.Info("plugz: Just write different processors")
	pp.Info("")
	pp.Info("Traditional: Test doubles, stubs, spies")
	pp.Info("plugz: Just different contract keys")
	pp.Info("")
