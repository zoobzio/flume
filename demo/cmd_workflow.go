...
func setupWorkflow() *plugz.Chain[WorkflowData] {
	...
	workflow := plugz.NewChain[WorkflowData]()
	workflow.Add(
		validationContract.Link(),
		enrichmentContract.Link(),
		persistenceContract.Link(),
...
enrichStage := plugz.GetContract[WorkflowData](EnrichmentStage)

// Add conditional stages
if user.Country == "EU" {
    gdprContract := plugz.GetContract[WorkflowData](GDPRStage)
