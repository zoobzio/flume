...
func chargeCard(p Payment) ([]byte, error) {
	...
	if p.Amount > p.CardLimit {
		...
		errorContract := plugz.GetContract[PaymentError](PaymentErrorHandlerV1)
...
paymentContract := plugz.GetContract[Payment](PaymentContractV1)
...
timeoutContract := plugz.GetContract[PaymentError](TimeoutHandlerV1)
...
fraudContract := plugz.GetContract[PaymentError](FraudHandlerV1)
