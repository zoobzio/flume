...
func TestTypeUniverseIsolation(t *testing.T) {
	...
	standardContract := plugz.GetContract[examples.Payment](standardKey)
	premiumContract := plugz.GetContract[examples.Payment](premiumKey)
	testContract := plugz.GetContract[examples.Payment](testKey)
...
func TestPipelineVersioning(t *testing.T) {
	...
	v1 := plugz.GetContract[examples.Payment](paymentKeyV1)
	v1.Register(
		plugz.Apply(examples.ValidatePayment),
	)

	// Register v2 pipeline - validation + fraud check
	v2 := plugz.GetContract[examples.Payment](paymentKeyV2)
	v2.Register(
		plugz.Apply(examples.ValidatePayment),
...
func TestChainComposition(t *testing.T) {
	...
	validationContract := plugz.GetContract[examples.Order](validatorKey)
	...
	enrichmentContract := plugz.GetContract[examples.Order](enrichmentKey)
	enrichmentContract.Register(func(o examples.Order) ([]byte, error) {
		// Add some metadata
		if o.CustomerID == "" {
			o.CustomerID = "GUEST"
		}
		return plugz.Encode(o)
	})

	// Create a chain
	chain := plugz.NewChain[examples.Order]()
	chain.Add(
...
func TestConcurrentPipelines(t *testing.T) {
	...
	contract := plugz.GetContract[examples.Payment](key)
