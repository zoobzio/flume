...
var allCmd = &cobra.Command{
	...
	Long:  `Runs through all plugz capability demonstrations with brief pauses between each.`,
	Run:   runAllDemos,
}
...
// It provides an overview of plugz concepts before diving into specific use cases.
func runAllDemos(cmd *cobra.Command, args []string) {
	...
	// Introduction
	pp.Section("🚀 plugz INTERACTIVE DEMONSTRATIONS")
	
	pp.SubSection("What is plugz?")
	pp.Info("plugz lets you build processing pipelines that are:")
...
contract := plugz.GetContract[DataType](myKey)
...
result, err := contract.Process(data)`)
	...
	pp.Code("shell", `plugz-demo security    # Deep dive into security patterns
plugz-demo universes   # Explore multi-tenant architectures`)
	
	pp.Info("")
	pp.Info("2. Try plugz in your own project:")
	pp.Code("shell", `go get github.com/your-org/plugz`)
