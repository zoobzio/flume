...
var securityCmd = &cobra.Command{
	...
	Long: `Demonstrates how plugz enables automatic security auditing with data redaction.
...
// It showcases how plugz can be used to build HIPAA/GDPR compliant systems
...
func runSecurityDemo(cmd *cobra.Command, args []string) {
	...
	pp.SubSection("🔧 How plugz Solves This")
...
auditContract := plugz.GetContract[AuditableData](AuditContractV1)
auditContract.Register(
	...
	auditContract := plugz.GetContract[examples.AuditableData](AuditContractV1)
	
	// Use adapters to convert processors to plugz.Processor format
	checkPermissions := plugz.Apply(examples.CheckPermissions)
	logAccess := plugz.Apply(examples.LogAccess)
	redactSensitive := plugz.Apply(examples.RedactSensitive)
	trackCompliance := plugz.Apply(examples.TrackCompliance)
...
func handlePatientDataRequest(userID string, patientData *User) {
    ...
    contract := plugz.GetContract[AuditableData](AuditContractV1)
...
func generateInvoice(patientID string) {
    ...
    auditPipeline := plugz.GetContract[AuditableData](AuditContractV1)
	...
	discoveredContract := plugz.GetContract[examples.AuditableData](AuditContractV1)
	...
	pp.SubSection("plugz Solution")
	pp.Info("✅ Just use the types:")
	pp.Info("  contract := plugz.GetContract[AuditableData](AuditContractV1)")
	pp.Info("")
