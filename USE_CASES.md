...
This document showcases real-world use cases for plugz. Don't just read the code - **run the live demos** to see the magic happen! Each use case has an interactive demonstration:
...
validationContract := plugz.GetContract[Order](validatorKey)
validationContract.Register(
    plugz.Apply(validateOrderID),
    plugz.Apply(validateItems),
    plugz.Apply(validateTotal),
)
```
...
auditContract := plugz.GetContract[AuditableData](auditKey)
auditContract.Register(
    plugz.Apply(checkPermissions),
    plugz.Apply(logAccess),
    plugz.Apply(redactSensitive),
)
```
...
transformContract := plugz.GetContract[TransformContext](transformKey)
transformContract.Register(
    plugz.Apply(parseCSV),
    plugz.Apply(validateEmail),
    plugz.Apply(enrichData),
)
```
...
func setupWorkflow() *plugz.Chain[WorkflowData] {
    // Validation stage
    validationContract := plugz.GetContract[WorkflowData](ValidationStage)
    validationContract.Register(
        plugz.Apply(func(w WorkflowData) (WorkflowData, error) {
            if w.User.Email == "" {
    ...
    // Enrichment stage
    enrichmentContract := plugz.GetContract[WorkflowData](EnrichmentStage)
    ...
    // Persistence stage
    persistenceContract := plugz.GetContract[WorkflowData](PersistenceStage)
    ...
    chain := plugz.NewChain[WorkflowData]()
    chain.Add(
...
middlewareContract := plugz.GetContract[Request](middlewareKey)
middlewareContract.Register(
    plugz.Apply(authenticate),
    plugz.Apply(rateLimit),
    plugz.Apply(logRequest),
)
```
...
paymentContract := plugz.GetContract[Payment](paymentKey)
paymentContract.Register(
    plugz.Apply(validatePayment),
    plugz.Apply(chargeCard),
    plugz.Apply(updateStatus),
)

const errorKey PaymentErrorKey = "v1"
errorContract := plugz.GetContract[PaymentError](errorKey)
errorContract.Register(
    plugz.Apply(categorizeError),
    plugz.Apply(notifyCustomer),
    plugz.Apply(attemptRecovery),
    plugz.Apply(auditLog),
)
```

### Key Insight: Type-Based Discovery

The error handling use case demonstrates the most powerful aspect of plugz: **complete decoupling through type-based discovery**. 
...
vA := plugz.GetContract[Payment](paymentKeyA)
vA.Register(plugz.Apply(chargeCard))

// Version B: Add validation
const paymentKeyB PaymentKey = "B"
vB := plugz.GetContract[Payment](paymentKeyB)
vB.Register(
    plugz.Apply(validateAmount),
    plugz.Apply(chargeCard),
    plugz.Apply(logPayment),
)

// Version C: Enterprise features
const paymentKeyC PaymentKey = "C"
vC := plugz.GetContract[Payment](paymentKeyC)
vC.Register(
    plugz.Apply(validateAmount),
    plugz.Apply(checkVelocity),
    plugz.Apply(fraudScore),
    plugz.Apply(routeProvider),
    plugz.Apply(chargeCard),
    plugz.Apply(notify),
    plugz.Apply(analytics),
)
...
This pattern shows how plugz enables:
...
prodContract := plugz.GetContract[EmailMessage](prodEmailKey)
prodContract.Register(
    plugz.Apply(validateEmail),
    plugz.Apply(sanitizeContent),
    plugz.Apply(applyTemplate),
    plugz.Apply(sendViaProvider),  // Real email sending
)

// Test pipeline - same types, different universe!
const testEmailKey EmailServiceKey = "test"
testContract := plugz.GetContract[EmailMessage](testEmailKey)
testContract.Register(
    plugz.Apply(validateEmail),    // Reuse real validation
    plugz.Apply(sanitizeContent),  // Reuse real sanitization
    plugz.Apply(testTemplate),     // Simplified for tests
    plugz.Apply(mockSend),         // Don't actually send!
)
...
hybridContract := plugz.GetContract[EmailMessage](hybridEmailKey)
hybridContract.Register(
    plugz.Apply(validateEmail),    // Real validation
    plugz.Apply(sanitizeContent),  // Real sanitization
    plugz.Apply(applyTemplate),    // Real templates
    plugz.Apply(mockSend),         // But don't send
)
```

### Key Insight: Tests Are Just Another Universe

Testing with plugz doesn't require:
...
Reading code is one thing, but seeing plugz in action is transformative. **Run the demos** - they're interactive, informative, and will help you truly understand the power of type-based pipelines:
