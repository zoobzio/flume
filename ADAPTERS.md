...
Adapters are the bridge between your business logic and plugz's byte-based pipeline system. They let you write simple functions that work with your types, while plugz handles all the serialization magic behind the scenes.
...
These key types help plugz identify and retrieve the correct pipeline for your data types.

## Why Adapters?

Under the hood, plugz pipelines work with bytes. This design enables:
...
contract := plugz.GetContract[User](userKey)
contract.Register(
    plugz.Validate(func(u User) error {
        if u.Age < 0 {
            return errors.New("invalid age")
        }
        return nil
    }),
    plugz.Transform(func(u User) User {
        u.Email = strings.ToLower(u.Email)
        return u
    }),
)
...
contract := plugz.GetContract[User](userKey)
contract.Register(plugz.Transform(addTimestamp))

// Your functions don't need to know about plugz!
...
contract := plugz.GetContract[User](userKey)
contract.Register(plugz.Transform(business.NormalizeUser))
```
...
contract := plugz.GetContract[User](userKey)
contract.Register(
    plugz.Apply(func(u User) (User, error) {
        if u.Email == "" {
...
contract := plugz.GetContract[User](userKey)
contract.Register(plugz.Apply(enrichUserFromAPI))
```
...
contract := plugz.GetContract[Order](orderKey)
contract.Register(
    plugz.Validate(func(o Order) error {
        if o.Total <= 0 {
...
userContract := plugz.GetContract[User](userKey)
userContract.Register(
    plugz.Validate(validators.ValidateAge),
    plugz.Validate(validators.ValidateEmail),
)
```
...
contract := plugz.GetContract[Order](orderKey)
contract.Register(
    plugz.Mutate(applyBulkDiscount, isBulkOrder),
    plugz.Mutate(applyHolidayPricing, isHolidaySeason),
)
```
...
contract := plugz.GetContract[User](userKey)
contract.Register(
    plugz.Effect(func(u User) error {
...
contract := plugz.GetContract[User](userKey)
contract.Register(
    plugz.Effect(notifyUserService),
    plugz.Effect(sendWelcomeEmail),
)
...
contract := plugz.GetContract[Order](orderKey)
contract.Register(
    plugz.Effect(func(o Order) error {
...
contract := plugz.GetContract[User](userKey)
contract.Register(
    plugz.Enrich(func(u User) (User, error) {
...
contract := plugz.GetContract[User](userKey)
contract.Register(plugz.Enrich(enrichWithPurchaseHistory))
```
...
```go
func Retry[T any](fn func(T) (T, error), maxAttempts int) plugz.Processor[T] {
    return func(value T) ([]byte, error) {
        var lastErr error
...
```go
func Cache[T any](fn func(T) (T, error), keyFn func(T) string, ttl time.Duration) plugz.Processor[T] {
    return func(value T) ([]byte, error) {
        key := keyFn(value)
...
```go
func Timeout[T any](fn func(T) (T, error), duration time.Duration) plugz.Processor[T] {
    return func(value T) ([]byte, error) {
...
func Batch[T Batchable](fn func([]T) error, size int, timeout time.Duration) plugz.Processor[T] {
    var (
        batch []T
        mu    sync.Mutex
        timer *time.Timer
...
Adapters are plugz's way of making byte-based pipelines feel natural. They handle all the serialization complexity so you can focus on writing clean, testable business logic. The built-in adapters cover most use cases, but creating custom adapters is straightforward when you need specialized behavior.

Remember: your business logic functions don't need to import plugz or know anything about pipelines. Keep them pure, simple, and focused on their specific task.