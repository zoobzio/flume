# plugz Performance Benchmarks

This directory contains comprehensive performance tests for plugz. All benchmarks are designed to measure real-world usage patterns and help you understand the performance characteristics of type-safe pipeline processing.
...
```go
plugz.Apply(examples.ValidateOrderID),
plugz.Apply(examples.ValidateItems), 
plugz.Apply(examples.ValidateTotal),
```
...
```go
plugz.Apply(examples.ValidatePayment),
plugz.Apply(examples.CheckFraud),
plugz.Apply(examples.UpdatePaymentStatus),
```
...
```go
plugz.Apply(examples.CheckPermissions),
plugz.Apply(examples.LogAccess),
plugz.Apply(examples.RedactSensitive),
plugz.Apply(examples.TrackCompliance),
```
...
```go
plugz.Apply(examples.ParseCSV),
plugz.Apply(examples.ValidateEmail),
plugz.Apply(examples.NormalizePhone),
plugz.Apply(examples.EnrichData),
```
...
contract.Register(
    plugz.Transform(step1),
    plugz.Transform(step2), 
    plugz.Transform(step3),
)
...
var userContract = plugz.GetContract[User](userKey)
...
- **plugz overhead**: ~200-500ns per operation
- **Trade-off**: Type safety + discoverability vs raw speed

### vs Interface-Based DI
- **plugz**: ~2-5x faster than reflection-heavy DI containers
- **plugz**: Zero startup time vs DI container initialization
- **plugz**: Predictable performance vs DI container complexity

### vs Manual Pipeline Code
- **plugz**: ~10-30% slower than hand-optimized pipeline code
- **plugz**: Much faster to develop and maintain
- **plugz**: Better type safety and error handling
...
- **CPU**: Dominated by business logic, not plugz overhead

**Recommendation**: plugz overhead is negligible for most real-world applications. The benefits of type safety, maintainability, and code organization far outweigh the small performance cost.
...
5. **Include baselines**: Compare with non-plugz alternatives when relevant
...
func BenchmarkYourScenario(b *testing.B) {
    ...
    contract := plugz.GetContract[YourType](testKey)
    contract.Register(
        plugz.Apply(yourProcessor),
    )
