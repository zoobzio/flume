...
❌ **No type-based lookup** - loses key plugz feature
...
1. **High-Performance Hot Paths**
   ...
   contract := plugz.NewSimpleContract[Data]()
...
2. **Concurrent Processing**
   ...
   contract := plugz.NewSimpleContract[Request]()
...
3. **Isolated Components**
   ...
   type Service struct {
       pipeline *plugz.SimpleContract[Order]
   }
   ```
...
2. **Dynamic Pipeline Selection**
   ...
   contract := plugz.GetContract[Data](key)
   ```
...
globalContract := plugz.GetContract[User](authKey)

// For performance-critical paths
fastContract := plugz.NewSimpleContract[User]()
