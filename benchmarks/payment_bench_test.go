...
func BenchmarkPaymentPipeline(b *testing.B) {
	...
	contract := plugz.GetContract[examples.Payment](paymentKey)
	contract.Register(
...
func BenchmarkPaymentErrorPipeline(b *testing.B) {
	...
	contract := plugz.GetContract[examples.PaymentError](errorKey)
	contract.Register(
		plugz.Apply(func(pe examples.PaymentError) (examples.PaymentError, error) {
			// Categorize error
